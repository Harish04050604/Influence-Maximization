...
# # -------------------------------
# def compute_influential_gain(node, seeds, G):
#     """Influential gain based on sum of edge weights to neighbors not in seeds."""
#     neighbors = set(G.neighbors(node)) - set(seeds)
#     return sum(G[node][nbr]['weight'] for nbr in neighbors)

# def compute_diversity_score(node, seeds, G):
#     """Diversity: average shortest path distance from existing seeds."""
#     if not seeds:
#         return 1.0  # First node has max diversity
#     lengths = nx.single_source_shortest_path_length(G, node)
#     distances = [lengths.get(s, len(G)) for s in seeds]  # default large distance if not reachable
#     avg_distance = sum(distances) / len(distances)
#     return 1 / (1 + avg_distance)  # normalize

# lambda_param = 0.7  # weight for influence vs diversity

# # -------------------------------
# # Step 4: Select top k seeds deterministically with scores
# # -------------------------------
# k = 5
# seeds = []
# seeds_info = []
...
#         inf_gain = compute_influential_gain(node, [s['node'] for s in seeds_info], G)
#         div_score = compute_diversity_score(node, [s['node'] for s in seeds_info], G)
...
# # -------------------------------
# # Step 5: Display seed set with scores
# # -------------------------------
...
# node_colors = ['red' if n in seeds else 'skyblue' for n in G.nodes()]
